...
from app.constants import GPT_3_5_TURBO_MODEL
...
class ChatCompletionBase(ABC):
    ...
    @abstractmethod
    def generate_completion(self, messages) -> str:
        """
        Abstract method to generate a completion based on a user's message and model.

         Args:
            message (list of dict): The user's chat history.
...
class GPTChatCompletion(ChatCompletionBase):
    ...
    def __init__(self):
        super().__init__(GPT_3_5_TURBO_MODEL)

    def generate_completion(self, messages) -> str:
        """
        Generate a completion based on a user's chat history using GPT-3.5 Turbo.
        """
        try:
